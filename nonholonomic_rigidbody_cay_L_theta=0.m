%% Nonholonomic Rigid Body SO(3) Left Cayley
...
% --- Group update (Cayley map)
function Rnext = f_fun(Rcur, omega, t)
    Rnext = Rcur * (eye(3) - t * hat(omega))^(-1) * (eye(3) + t * hat(omega));
end

% --- Momentum update (coadjoint action)
function pinext = h_fun(omega, picur, t)
    pinext = (eye(3) - t * hat(omega))^(-1) * (eye(3) + t * hat(omega)) * picur;
end
...
function res = g_fun(omega2, picur, I, t)
    ...
    full_res = 2 * (1 + theta^2)^(-1) * (eye(3) + t * hat(omega)) * picur ...
               - t * I * omega;
